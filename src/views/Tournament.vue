<template>
  <div class="about">
    <h1 style="text-align: center;">Astroworld Tracks</h1>
...
export default {
  data() {
    return {
      ...
      track2albumID: '4PWBTB6NYSKQwfo79I3prg',
      currentindex: 0,
      currentindex2: 0,
      astroworldTracks: [],
      rodeoTracks: [],
  ...
  methods: {
    ...
    async renderAstroworld() {
      ...
      const tracks = await this.getTracks(token, this.albumId);
      this.astroworldTracks = tracks;
      this.renderTracks(); // Call renderTracks here
    },
    async renderRodeo() {
      const token = await this.getToken();
      const tracks = await this.getTracks(token, this.track2albumID);
